...
5. [API](#5-api)
6. [Blue-Green Deployment (search-service)](#6-blue-green-deployment-search-service)
7. [Data Flows](#7-data-flows)
8. [Storage and Search Strategy](#8-storage-and-search-strategy)
9. [Consistency Model and Trade-offs](#9-consistency-model-and-trade-offs)
10. [Multi-Tenancy and Security](#10-multi-tenancy-and-security)
11. [Rate Limiting and Resilience](#11-rate-limiting-and-resilience)
12. [Production Readiness Analysis](#12-production-readiness-analysis)
13. [Resilience](#13-resilience)
14. [Observability](#14-observability)
15. [Performance Strategy](#15-performance-strategy)
16. [Operations and Availability](#16-operations-and-availability)
17. [Code Quality](#17-code-quality)
18. [Experience Showcase](#18-experience-showcase)
19. [AI Tool Usage](#19-ai-tool-usage)
20. [Key Design Decisions at a Glance](#20-key-design-decisions-at-a-glance)
21. [Assessment Alignment](#21-assessment-alignment)
22. [Assumptions and Prototype Limitations](#22-assumptions-and-prototype-limitations)
...
- **Search Service** – tenant-filtered full-text search using Elasticsearch/BM25 with a local Caffeine cache, deployed
  blue-green behind nginx.
...
- `search-service`: tenant-filtered full-text search with Caffeine query cache; runs blue-green in Docker.
- `common`: small shared contracts only (Kafka event and headers).

### Component responsibilities and scaling model
...
| Search Service   | Query orchestration, tenant filtering, caching         | Blue-green pair behind nginx (see §6); horizontal replicas per color in production |
...
`active.conf` decides which, and `scripts/switch-search-color.sh` flips it with zero downtime (details
in [§6](#6-blue-green-deployment-search-service)).
...
`search-service` runs blue-green behind an nginx proxy (see [§6](#6-blue-green-deployment-search-service)); `gateway`
still reaches it at `http://search-service:8083` without any changes.
...
Performs a tenant-filtered full-text query against Elasticsearch and returns relevance-ranked results.
...
For `search-service` specifically, `8083` goes through the nginx proxy to whichever color is active; hit `8091`/`8092`to
check blue/green directly:
...
## 6. Blue-Green Deployment (search-service)

`search-service` runs as two identical containers, `search-service-blue` and `search-service-green`, fronted by an
`nginx` reverse proxy that owns the shared `search-service` network identity and port (`8083`) — see
the [architecture diagram](#3-architecture) above. `gateway`'s `SEARCH_SERVICE_URL` always points at the proxy, so
switching colors never requires touching the gateway.
...
`docker compose up --build` brings up both colors plus the proxy alongside the rest of the stack:

![Containers running: postgres, redis, elasticsearch, zookeeper, kafka, search-service-blue, search-service-green, search-service-proxy, indexer-service, document-service, gateway](img_2.png)

This is a deliberately minimal, docker-compose-appropriate implementation of the blue-green pattern. In production this
would be replaced by a platform-native mechanism (Kubernetes Service/Ingress cutover, an ALB/NLB target-group swap, or a
service mesh traffic split) with automated canary analysis and rollback rather than a manually-invoked script, and would
extend to every stateless service, not just `search-service`.

---

## 7. Data Flows

### 7.1 Document Indexing Flow
...
The Elasticsearch query combines full-text matching with a mandatory tenant filter. The tenant filter is implemented as
a **keyword field**, allowing exact filtering without analyzing the tenant identifier.

---

## 8. Storage and Search Strategy
...
Redis stores shared gateway rate-limit state. This prevents a tenant's effective limit from multiplying when gateway
replicas are added.

### Caffeine – Local Search Cache

The prototype uses a short-TTL local cache in Search Service. Cache keys include the tenant identifier and normalized
query, preventing cross-tenant cache leakage.
...
`tenantId` is carried in `X-Tenant-Id`. The gateway requires it and all downstream APIs scope data by tenant. Tenant
...
Document Service validates that the requested document belongs to the authenticated tenant. Search Service always
applies a tenant filter.

### Search layer

`tenantId` is mapped as an Elasticsearch `keyword` field and used in an exact filter, preventing analyzed text matching
from weakening isolation. Conceptually:
...
Rate limiting is enforced at the API Gateway because it is the common external policy boundary. Redis stores the
token-bucket state so the limit remains consistent across multiple gateway instances.
...
Blue-green is already implemented for `search-service` rather than left as a purely aspirational production item —
see [§6](#6-blue-green-deployment-search-service) for the full design and run-book. In production this pattern would
extend to every stateless service via a platform-native mechanism (Kubernetes, an ALB/NLB target-group swap, or a
service mesh traffic split) with automated canary analysis.
...
| Deployment failure            | Blue-green/canary deployment + rollback (implemented for `search-service`, see §6) |
...
- rolling/blue-green deployments (blue-green demonstrated for `search-service` in this prototype, see §6; production
...
## 18. Experience Showcase

### 18.1 Similar Distributed System
...
### 18.3 Critical Production Incident
...
- implementing and documenting the blue-green deployment for `search-service`;
...
## 20. Key Design Decisions at a Glance
...
## 22. Assumptions and Prototype Limitations
...
- Blue-green deployment implemented for `search-service` (nginx-based, script-driven cutover — see §6), rather than only
  described as a future concern.
...
- Blue-green/canary deployments at the orchestration layer (Kubernetes/service mesh/load-balancer target-group swap)with
  automated canary analysis, extending the manual nginx-based pattern already demonstrated for `search-service` to every
  stateless service.
